...
import { WishlistForm, WishlistFormProps } from "@fpapackages/fpa-wishlist-form";
import { useFooterConfig } from '../utils/useFooterConfig';
import { MenuLink } from '../components/Footer';

const AddWishlist = (data: any) => {
  ...
  const formData = {
    ...
    sendWishlist: data.data.sendWishlist,
    privacyPolicy: data.data.privacyPolicy,
    promotionalOffers: data.data.promotionalOffers,
    onFormSubmit: (data: WishlistFormProps) => {
    },
  }
  return (
    <section className="max-w-md mx-auto rounded-xl overflow-hidden md:max-w-2xl">
      <div className="md:flex justify-center">
      <WishlistForm {...formData}></WishlistForm>
    </div>
    </section>
  );
};
